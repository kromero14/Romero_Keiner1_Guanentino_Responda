<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title>¡Guanentino, responda!</title>
    <meta content="width=device-width, initial-scale=1.0" name="viewport">
    <link rel="icon" type="image/jpg" href="img/favico.jpg">


    <!-- Favicon -->
    <link href="img/favicon.ico" rel="icon">

    <!-- Google Font -->
    <link rel="preconnect" href="https://fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@200;400&family=Roboto:wght@400;500;700&display=swap" rel="stylesheet"> 

    <!-- Font Awesome -->
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.10.0/css/all.min.css" rel="stylesheet">

    <!-- Libraries Stylesheet -->
    <link href="lib/owlcarousel/assets/owl.carousel.min.css" rel="stylesheet">
    <link href="lib/tempusdominus/css/tempusdominus-bootstrap-4.min.css" rel="stylesheet" />

    <!-- Customized Bootstrap Stylesheet -->
    <link href="style23.css" rel="stylesheet">
</head>

<body>
    <!-- Navbar Start -->
    <div class="container-fluid p-0 nav-bar">
        <nav class="navbar navbar-expand-lg bg-none navbar-dark py-3">
            <a href="index.html" class="navbar-brand px-lg-4 m-0">
                <h1 class="m-0 display-4 text-uppercase text-white">¡Guanentino!</h1>
            </a>
            <button type="button" class="navbar-toggler" data-toggle="collapse" data-target="#navbarCollapse">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse justify-content-between" id="navbarCollapse">
                <div class="navbar-nav ml-auto p-4">
                    <!-- Menú modificado con tus botones -->
                    <a href="generalidades.php" class="nav-item nav-link">Colegio Guanentá</a>
                    <a href="informacion_saber.php" class="nav-item nav-link">Información Saber 11</a>
                    <a href="home_inicio_sesion.php" class="nav-item nav-link">Iniciar Sesión</a>
                </div>
            </div>
        </nav>
    </div>
    <!-- Navbar End -->

    <!-- Carrusel de la plantilla (fondo principal) -->
    <div class="container-fluid p-0 mb-5">
        <div id="header-carousel" class="carousel slide" data-ride="carousel">
            <div class="carousel-inner">
                <div class="carousel-item active">
                    <img class="w-100" src="img/Colegio.jpg" alt="Image">
                    <div class="carousel-caption d-flex flex-column align-items-center justify-content-center">
                        <h2 class="text-white font-weight-medium m-0">Bienvenido a</h2>
                        <h1 class="display-1 text-white m-0">¡Guanentino, Responda!</h1>
                        <h2 class="text-white m-0">Proyecto educativo</h2>
                    </div>
                </div>
                <div class="carousel-item">
                    <img class="w-100" src="img/Colegio.jpg" alt="Image">
                    <div class="carousel-caption d-flex flex-column align-items-center justify-content-center">
                        <h2 class="text-white font-weight-medium m-0">Preparación para</h2>
                        <h1 class="display-1 text-white m-0">Pruebas Saber 11</h1>
                        <h2 class="text-white m-0">Colegio San José de Guanentá</h2>
                    </div>
                </div>
            </div>
            <a class="carousel-control-prev" href="#header-carousel" role="button" data-slide="prev">
                <div class="btn btn-dark" style="width: 45px; height: 45px;">
                    <span class="carousel-control-prev-icon mb-n2"></span>
                </div>
            </a>
            <a class="carousel-control-next" href="#header-carousel" role="button" data-slide="next">
                <div class="btn btn-dark" style="width: 45px; height: 45px;">
                    <span class="carousel-control-next-icon mb-n2"></span>
                </div>
            </a>
        </div>
    </div>
    <!-- Carrusel End -->
     <center>
    <h2>Información de nuestro proyecto</h2>
        <img src="img/favico.jpg" alt="" height="350" width="500">
        <br>
        <br>
<p><strong>Descripción:</strong> Estrategia del Colegio Guanentá para preparar a los estudiantes de 10° y 11° en el examen Saber 11.</p>

<p><strong>Metodología:</strong> Simulacros repetidos tipo ICFES, talleres de refuerzo, retroalimentación docente y uso de plataforma virtual.</p>

<p><strong>Participación:</strong> Estudiantes de grados superiores con acompañamiento de docentes y coordinadores.</p>

<p><strong>Seguimiento:</strong> Registro de puntajes, análisis de avances y planes de refuerzo individual.</p>

<p><strong>Impacto:</strong> Mejora en los resultados, confianza estudiantil y fortalecimiento académico institucional.</p>

<br>
</center>
    <!-- Footer Start -->
    <footer class="bg-dark text-white text-center py-4">
        <p>&copy; 2025 Proyecto ¡Guanentino, Responda! - Colegio San José de Guanentá.</p>
        <p>Desarrollador: Keiner Santiago Romero Delgado - Especialidad en Sistemas</p>
    </footer>
    <!-- Footer End -->

    <!-- JavaScript Libraries -->
    <script src="https://code.jquery.com/jquery-3.4.1.min.js"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.bundle.min.js"></script>
    <script src="lib/easing/easing.min.js"></script>
    <script src="lib/waypoints/waypoints.min.js"></script>
    <script src="lib/owlcarousel/owl.carousel.min.js"></script>
    <script src="lib/tempusdominus/js/moment.min.js"></script>
    <script src="lib/tempusdominus/js/moment-timezone.min.js"></script>
    <script src="lib/tempusdominus/js/tempusdominus-bootstrap-4.min.js"></script>

    <!-- Template Javascript -->
    <script src="js/main.js"></script>
</body>
</html>
